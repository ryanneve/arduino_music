...
static uint16_t MUSIC_PWM = 500; // Originally 500
...
void Music::playNote(uint16_t melody_idx, float frequency,uint16_t music_tempo) {
  ...
  for (melody_idx = 0; melody_idx < _note_count; melody_idx++) {
    ...
    for (n_idx=0;n_idx<SCALE_LEN;n_idx++) { // See what melody_idx this is
      if (MUSIC_NOTES[n_idx] == _melody[melody_idx*2 + 1]) {
        ...
        playNote(melody_idx,MUSIC_FREQUENCY[n_idx],music_tempo);
      }
    }
  }
}
